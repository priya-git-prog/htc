class CompaniesController < ApplicationController
  ...
  def generate_bill
    if params[:ticket_ids].present?
      bill_service = BillingService.new params[:ticket_ids]
      bill_service.generate_and_send_bill
    else 
      redirect_back(fallback_location: root_path, notice: "No tickets selected")
    end
  end
  ...
  private
    ...
    def company_params
      params.require(:company).permit(:name, :branch)
    end
  end
